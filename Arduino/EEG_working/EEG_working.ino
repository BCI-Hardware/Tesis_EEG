...
void setup() {
  ...
  fWREG(BIAS_SENSP, 0x03); // Bias IN1P
  fWREG(BIAS_SENSN, 0x03);
  ...
  // Acativate this for reference mode
  //fWREG(MISC1, 0x20);
...
void loop() {
  ...
  for (int i=0; i < active_channels; i++){
    ...
    float value_to_send =(float)(aux_ch*4500/(pow(2, 23) - 1));

    byte * b = (byte *) &value_to_send;
